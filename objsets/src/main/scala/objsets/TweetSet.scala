...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    if(!this.exists) that
    else if(!that.exists) new NonEmpty(this.elem, this.left, this.right )
    else if(!that.exists && !that.exists) new Empty
    else this.filterAcc(p => !p.text.isEmpty, that)
  }

  def mostRetweeted: Tweet = {
    var x = this.elem
    foreach(t => {
      if(t.retweets > elem.retweets) x = t
      else x = elem
    })
    if(!this.exists) throw new NoSuchElementException
    else x
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter({
      p => google.exists(x => p.text.contains(x))
  })

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter({
    p => apple.exists(x => p.text.contains(x))
  })
  /**
